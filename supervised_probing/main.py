...
from utils import (Log, log_system_info, hf_login, TextDataset, probe_classification_local, extract_hidden_states)
...
os.makedirs(f'{model_short_name}/{args.task_type}', exist_ok=True)
...
def get_data(data_type = args.task_type):
    if(data_type=='twitter'):
        ...
        train_data = pd.read_csv('datasets/dailydialog_train_processed_filtered.json')
    ...
    return dataloader_train, dataloader_test, labels_train, labels_test
...
dataloader_train, dataloader_test, labels_train, labels_test = get_data(data_type = args.task_type)
...
if args.extract_hidden_states:
    ...
    all_hidden_states = extract_hidden_states(dataloader_train, tokenizer, model, logger, extraction_locs=extraction_locs, extraction_layers=extraction_layers, extraction_tokens = extraction_tokens,concat=False)
    all_hidden_states_test = extract_hidden_states(dataloader_test, tokenizer, model, logger, extraction_locs=extraction_locs, extraction_layers=extraction_layers, extraction_tokens = extraction_tokens,concat=False)
...
if args.emotion_probing:
    ...
    for i, layer in tqdm(enumerate (extraction_layers), total=len(extraction_layers)):
        ...
        for j, loc in enumerate (extraction_locs):
            ...
            for k, token in enumerate (extraction_tokens):
                results[layer][loc][token] = probe_classification_local(args.given_path,layer, all_hidden_states[:, i, j, k], labels_train[:, 0],all_hidden_states_test[:, i, j, k], labels_test[:, 0], return_weights=False,probe_type=args.probe_cls)
    ...
    torch.save(results, f'{model_short_name}/{args.task_type}/emotion_probing_layers_{extraction_layers}_locs_{extraction_locs}_tokens_{extraction_tokens}_{args.prompt_version}.pt')

    with open('plot_res.json','a') as f:
